...
const mergeConfigs = (rootPath, mmtConfig, mmtWebpackConfigExtensions) => {
  const configurableConfig = configurableWebpackValues(rootPath, mmtConfig)
...
const configurableWebpackValues = (rootPath, { entry }) => {
  const entryPathRelativeToProject = path.resolve(rootPath, entry)
  const outputPathRelativeToProject = path.resolve(rootPath, 'build/assets')
...
const start = async () => {
  ...
  try {
    ...
    logError('Unable to find config file — config/mmt.config.js')
    return
  }
  ...
  try {
    webpackConfig = mergeConfigs(rootPath, mmtConfig, mmtWebpackConfigExtensions)
  } catch (error) {
